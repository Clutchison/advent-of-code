...
public enum Operation {
    ...
    Operation(int code, UnaryOperation op) {
        this.op = op;
        this.code = code;
    }

    public Codes apply(Codes codes) {
        return op.apply(codes);
    }

    static {
        Stream.of(Operation.values()).forEach(op -> map.put(op.code, op));
    }

    public static Operation fromCode(double code) {
